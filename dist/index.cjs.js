  ...
  constructor(code, message, options, ...contexts) {
    ...
    for (const context of contexts) {
      for (const key in context) {
        ...
        this[key] = Buffer$1.isBuffer(value)
...
const init_state = function (options) {
  return {
    ...
    escaping: false,
    escapeIsQuote:
      Buffer$1.isBuffer(options.escape) &&
      Buffer$1.isBuffer(options.quote) &&
      Buffer$1.compare(options.escape, options.quote) === 0,
    ...
    trimChars: [
      Buffer$1.from(" ", options.encoding)[0],
      Buffer$1.from("\t", options.encoding)[0],
    ],
    wasQuoting: false,
    wasRowDelimiter: false,
    timchars: [
      Buffer$1.from(Buffer$1.from([cr$1], "utf8").toString(), options.encoding),
      Buffer$1.from(Buffer$1.from([nl$1], "utf8").toString(), options.encoding),
      Buffer$1.from(Buffer$1.from([np], "utf8").toString(), options.encoding),
      Buffer$1.from(Buffer$1.from([space], "utf8").toString(), options.encoding),
      Buffer$1.from(Buffer$1.from([tab], "utf8").toString(), options.encoding),
    ],
  };
};
...
const normalize_options = function (opts) {
  ...
  if (options.record_delimiter === undefined) {
    ...
    Buffer$1.isBuffer(options.record_delimiter)
  ) {
...
class HsbcParser {
    ...
    parse(fileContent) {
        try {
            const workbook = readSync(fileContent, { type: 'buffer' });
